...
def main():
    ...
    speedup_data = {typ: ([], []) for typ in data.keys()}
    efficiency_data = {typ: ([], []) for typ in data.keys()}

    for typ, (nb_list, temps_list) in data.items():
        for nb, temps in zip(nb_list, temps_list):
            speedup = seq_time / temps
            efficiency = speedup / nb
            speedup_data[typ][0].append(nb)
            speedup_data[typ][1].append(speedup)
            efficiency_data[typ][0].append(nb)
            efficiency_data[typ][1].append(efficiency)
            print(f"{typ} {nb} : speedup = {speedup:.2f}, efficiency = {efficiency:.2f}")
    ...
    for typ, (nb_list, speedup_list) in speedup_data.items():
        if nb_list:
            # Trier par nombre de processus
            tri = sorted(zip(nb_list, speedup_list))
            nb_trie, speedup_trie = zip(*tri)
            style = styles.get(typ, {'color': 'black', 'marker': 'x', 'label': typ})
            plt.plot(nb_trie, speedup_trie,
                     marker=style['marker'],
                     linestyle='-',
                     linewidth=2,
                     markersize=8,
    ...
    plt.ylabel('Speedup')
    plt.title('Speedup selon la méthode de découpage')
    plt.grid(True, linestyle='--', alpha=0.7)
    plt.legend()
    plt.tight_layout()
    plt.savefig('speedup.png', dpi=150)
    print("\nGraphique de speedup sauvegardé sous 'speedup.png'.")

    # Tracer les courbes d'efficacité
    plt.figure(figsize=(12, 8))

    for typ, (nb_list, efficiency_list) in efficiency_data.items():
        if nb_list:
            # Trier par nombre de processus
            tri = sorted(zip(nb_list, efficiency_list))
            nb_trie, efficiency_trie = zip(*tri)
            style = styles.get(typ, {'color': 'black', 'marker': 'x', 'label': typ})
            plt.plot(nb_trie, efficiency_trie,
                     marker=style['marker'],
                     linestyle='-',
                     linewidth=2,
                     markersize=8,
    ...
    plt.title('Efficacité selon la méthode de découpage')
    plt.grid(True, linestyle='--', alpha=0.7)
    plt.legend()
    plt.tight_layout()
    plt.savefig('efficiency.png', dpi=150)
    print("Graphique d'efficacité sauvegardé sous 'efficiency.png'.")
