...
class Grille:
    ...
    def exchange_ghost_columns(self, comm):
        ...
        if size == 2:
            # Cas spécial 2 processus : on utilise Sendrecv pour éviter les deadlocks
            send_left = self.cells[:, -2].copy()  # Copie pour assurer la contiguïté
            recv_left = np.empty(self.dimensions[0], dtype=np.uint8)
            comm.Sendrecv(sendbuf=send_left, dest=left, recvbuf=recv_left, source=left)
            self.cells[:, 0] = recv_left

            send_right = self.cells[:, 1].copy()  # Copie pour assurer la contiguïté
